...
uint32_t setup_gpu(){
  ...
  ret_cuda = cuDeviceGetName(cuda_device_name, 32, cu_device);
  ...
  gpu_events.num_events = 0;
  gpu_events.event = NULL;
...
uint32_t gpu_events_all(){
  ...
  gpu_events.event = event_ids;
  gpu_events.num_events = num_events_tot;
  // de-init
  free(domain_ids);
...
#ifdef __JETSON_AGX_XAVIER
  gpu_events.num_events = num_events;
  gpu_events.event = (gpu_event_t *)malloc(num_events * sizeof(gpu_event_t));
  if (gpu_events.event == NULL) {
    printf("%s:%d: failed to allocate memory.\n", __FILE__, __LINE__);
    exit(1);
  }
  for (int i = 0; i < num_events; i++){
    gpu_events.event[i] = events[i];
  }
  // create CUPTI event group sets (i.e., sets of CUPTI event groups)
  print_gpu_events();
...
uint32_t gpu_events_from_config(char *config_file){
  ...
  // set events
  gpu_events.num_events = events_config.gpu_events_freq_config[f].num_events;
  gpu_events.event = (gpu_event_t *)malloc(gpu_events.num_events * sizeof(gpu_event_t));
  for (int e = 0; e < events_config.gpu_events_freq_config[f].num_events; e++) {
    gpu_events.event[e] = events_config.gpu_events_freq_config[f].event[e];
  }
  // create CUPTI event group sets (i.e., sets of CUPTI event groups)
  print_gpu_events();
  uint32_t num_sets = cupti_create_event_group_sets(events_config.gpu_events_freq_config[f].event, events_config.gpu_events_freq_config[f].num_events);
  // free events_config
...
void parse_gpu_events_json(char *config_file, gpu_events_config_t *events_config){
  ...
  for (int f = 0; f < num_frequencies_json; f++) {
    ...
    events_config->gpu_events_freq_config[f].num_events = num_events_json;
    events_config->gpu_events_freq_config[f].event = (gpu_event_t *)malloc(num_events_json * sizeof(gpu_event_t));
    if (events_config->gpu_events_freq_config[f].event == NULL) {
      printf("%s:%d: failed to allocate memory.\n", __FILE__, __LINE__);
      exit(1);
    }
    for (int e = 0; e < num_events_json; e++){
      ...
      events_config->gpu_events_freq_config[f].event[e] = event;
    }
  }
}
...
static void free_events_config(gpu_events_config_t *events_config){
  for (int f = 0; f < events_config->num_freqs; f++) {
    free(events_config->gpu_events_freq_config[f].event);
  }
