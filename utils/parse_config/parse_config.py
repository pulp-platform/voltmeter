...
if __name__ == '__main__':
    ...
    with open(OUTPUT_MK, 'w') as f:
        ...
        f.write('# Content parsed from {}\n\n'.format(CONFIG_YML))
        f.write('-include ./config.mk\n\n')
        f.write('# Voltmeter platfrom and compilation parameter\n')
        for key in [k for k in config if k != 'arguments']:
            for param in config[key]:
                if type(config[key][param]) is bool:
                    f.write('{} := {}\n'.format(param, int(config[key][param])))
                elif type(config[key][param]) is list:
                    f.write('{} := {}\n'.format(param, ' '.join(map(str, config[key][param]))))
                else:
        ...
        for key in config['arguments']:
            ...
            if key == 'benchmarks':
                ...
                # '\\\n' are only used to make the generated Makefrag more readable
                f.write('benchmarks := $$\' \\\n')
